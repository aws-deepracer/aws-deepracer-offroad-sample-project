...
def api_set_drive_mode():
    """API to toggle the drive mode between Autonomous/Manual/Offroad mode.
...
@VEHICLE_CONTROL_BLUEPRINT.route("/api/offroad_max_speed", methods=["PUT", "POST"])
def max_offroad_speed():
    ...
    """
    webserver_node = webserver_publisher_node.get_webserver_node()
    offroad_speed = request.json.get("throttle")
    if offroad_speed is None:
        return jsonify({"success": False, "reason": "value must be set."})
    webserver_node.get_logger().info(f"Setting max offroad speed to {offroad_speed}%")
    try:
        set_speed_req = SetMaxSpeedSrv.Request()
        set_speed_req.max_speed_pct = float(offroad_speed / 100)
        set_speed_res = call_service_sync(webserver_node.set_offroad_max_speed_cli, set_speed_req)
...
def control_modes_available():
    """API to call the GetCtrlModesCountSrv service to get the list of available modes
       in ctrl_pkg (autonomous/manual/calibration/followtheleader/offroad).
