...
namespace SysCtrl {
    ...
    OffroadDriveCtrl::OffroadDriveCtrl(std::shared_ptr<rclcpp::Node> ctrlNodePtr, const std::string &subName)
       : isActive_(false)
     {
        ctrlNode = ctrlNodePtr;
        // Subscribe to the appropriate servo topic.
        ...
        servoSub_ = ctrlNode->create_subscription<deepracer_interfaces_pkg::msg::ServoCtrlMsg>(subName,
                                                                                               qos,
                                                                                               std::bind(&OffroadDriveCtrl::servoCB,
                                                                                                         this,
                                                                                                         std::placeholders::_1),
                                                                                               rclcpp::SubscriptionOptions(),
                                                                                               servoMsgStrategy);
    ...
    void OffroadDriveCtrl::servoCB(const deepracer_interfaces_pkg::msg::ServoCtrlMsg::SharedPtr msg) {
        if(!isActive_ || !servoPub_) {
            return;
        }
        auto servoMsg = deepracer_interfaces_pkg::msg::ServoCtrlMsg();
    ...
    bool OffroadDriveCtrl::loadModelReq(int requestSeqNum, std::string modelName, std::vector<int> modelMetadataSensors, 
                                     int trainingAlgorithm, int actionSpaceType, std::string imgFormat,
                                     int width, int height, int numChannels, 
                                     int lidarChannels, int platform, int task, int preProcess) {
        (void)requestSeqNum;
        ...
        (void)task;
        (void)preProcess;
        RCLCPP_ERROR(ctrlNode->get_logger(), "Cannot load model in offroad mode");
        return false;
    }

    void OffroadDriveCtrl::setStateActive(bool isActive) {
        isActive_ = isActive;
        // Stop car and straighten wheels when starting or stopping this state
        auto servoMsg = deepracer_interfaces_pkg::msg::ServoCtrlMsg();
        servoMsg.angle = 0.0;
    ...
    std::string OffroadDriveCtrl::getLoadModelStatus() {
        RCLCPP_ERROR(ctrlNode->get_logger(), "Cannot get load model status in offroad mode.");
        return "error";
    }

    void OffroadDriveCtrl::getCalibration(int type, std::vector<int> &cal) {
        (void)type;
        (void)cal;
        RCLCPP_ERROR(ctrlNode->get_logger(), "Calibration information not available in offroad mode");
    }

    void OffroadDriveCtrl::setCalibration(int type, const std::vector<int> & cal) {
        (void)type;
        (void)cal;
        RCLCPP_ERROR(ctrlNode->get_logger(), "Cannot set calibrations in offroad mode");
    }

    void OffroadDriveCtrl::getLedValue(std::vector<int> &ledValuesMap) {
        (void)ledValuesMap;
        RCLCPP_ERROR(ctrlNode->get_logger(), "Cannot get LED values in offroad mode");
    }

    void OffroadDriveCtrl::setLedValue(int redPwm, int greenPwm, int bluePwm) {
        (void)redPwm;
        (void)greenPwm;
        (void)bluePwm;
        RCLCPP_ERROR(ctrlNode->get_logger(), "Cannot set LED values in offroad mode");
    }

    void OffroadDriveCtrl::setConstantThrottle(float throttle) {
        (void)throttle;
        RCLCPP_ERROR(ctrlNode->get_logger(), "Cannot set constant throttle in offroad mode");
    }
}
