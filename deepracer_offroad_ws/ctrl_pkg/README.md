# AWS DeepRacer Control Package for DeepRacer Offroad

## Overview

The DeepRacer Control ROS package creates the *ctrl_node* which is part of the DeepRacer Offroad sample project and will be launched from the deepracer_offroad_launcher. This package was extended and modified from the DeepRacer Control ROS package developed for the core application. For more information about the DeepRacer Offroad sample project, see [DeepRacer Offroad sample project](https://github.com/aws-deepracer/aws-deepracer-offroad-sample-project).

This is the main node with services exposed to be used by webserver backend API calls. This node in the AWS DeepRacer application manages the different mode of the device [manual, autonomous, calibration]. It allows us to maintain the device in a single mode at any point of time so that the overlapping functionalities (like servo messages, etc) are not conflicting each other. In this package, an additional mode **offroad** has been added to support the DeepRacer Offroad sample project.
...
The ctrl_node provides basic system level functionality for the AWS DeepRacer application and DeepRacer Offroad sample project to work. Although the node is built to work with the AWS DeepRacer application and DeepRacer Offroad sample project, it can be run independently for development/testing/debugging purposes.
