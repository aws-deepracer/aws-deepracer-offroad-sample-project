...
namespace SysCtrl {
    ...
    class OffroadDriveCtrl : public CtrlStateBase
    {
    /// Control state for offroad driving.
    public:
        /// @param nodeHdl Reference to the node handler of the control node.
        /// @param subName Topic to subscribe to for servo messages.
        OffroadDriveCtrl(std::shared_ptr<rclcpp::Node> ctrlNodePtr, const std::string &subName);
        virtual ~OffroadDriveCtrl() = default;
